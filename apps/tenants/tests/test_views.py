...
class TenantDomainTests(TestCase):
    ...
    def setUp(self):
        ...
        self.site = Site.objects.create(name=f"a.{self.tenant_root_domain}", domain=f"a.{self.tenant_root_domain}")
        ...
        self.site_not_linked = Site.objects.create(name=f"notlinked.{self.tenant_root_domain}",
    ...
    def test_tenant_root_domain_should_be_accessible(self):
        ...
        response = self.client.get(self.home_url, HTTP_HOST=self.tenant_root_domain)
        self.assertEqual(response.status_code, 200)

    def test_tenant_marketing_domain_should_be_accessible(self):
        """The marketing page should be accessible."""
        response = self.client.get(self.home_url, HTTP_HOST=self.marketing_page.domain)
        self.assertEqual(response.status_code, 200)
    ...
    def test_tenant_not_existing_domain_should_give_not_found_error(self):
        ...
        self.assertEqual(response.status_code, 200)  # TODO: Is this a problem? But thats common Django behavior.
    ...
    def test_tenant_not_existing_site_should_give_not_found_error(self):
        ...
        response = self.client.get(self.home_url, HTTP_HOST=f'notexisting.{self.tenant_root_domain}')
        self.assertEqual(response.status_code, 404)

    @pytest.mark.skip(reason="This should probably give a 404 - but it gives a 200 now")
    ...
    def test_tenant_secret_page_on_root_domain_should_not_be_accessible(self):
        """The marketing page should be accessible."""
        response = self.client.get(self.secret_url, HTTP_HOST=self.tenant_root_domain)
    ...
    @pytest.mark.skip(reason="This should probably give a 404 - but it gives a 403 now")
    ...
    def test_tenant_secret_page_on_not_existing_site_should_give_not_found_error(self):
        """A non-existing subdomain of the tenant root domain should throw an error."""
        response = self.client.get(self.secret_url, HTTP_HOST=f'notexisting.{self.tenant_root_domain}')
        self.assertEqual(response.status_code, 404)
